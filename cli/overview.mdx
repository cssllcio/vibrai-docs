...
<Note>
`shell` is intercepted before the command parser, so it does not appear in the generated Reference section or in `vibrai llms` output. `vibrai shell --help` is the authority on it.
</Note>
