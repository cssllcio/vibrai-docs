...
Commands are organized into **branches** (subcommand groups) and a few top-level commands:
...
| `vibrai follow-action` | Get and set follow-action config |
...
Run `vibrai --help` to see the full tree. Run `vibrai <branch> --help` for subcommand detail.
