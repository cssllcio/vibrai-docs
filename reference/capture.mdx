...
<ParamField path="--merge-with">
  Overlay section metadata (category, tension, modifier, variation, scale overrides) AND Templates[0].Automation from this prior .vibrai onto the captured project. Sections matched by id, falling back to position. Scene names always come from the captured set.
</ParamField>
