...
```bash
vibrai track auto-number <state> --persist --write-project --project <PATH>
```

<ParamField path="state" required>
  on or off
</ParamField>
<ParamField path="--persist">
...
<ParamField path="write_project" type="Boolean">
  Write the .vibrai file to disk.
</ParamField>
