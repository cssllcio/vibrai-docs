...
```bash
vibrai generate --project <PATH> --force --clear-first --clear-clips-first --dry-run --seed <VALUE> --allow-skip --with-instruments --with-automation --stock-only --on-existing-tracks <MODE> --keep-empty-clips --session-only --create-missing-tracks --swing <PCT> --humanize <PCT> --ornaments <PCT> --dropout <PCT> --gate <PCT> --bass-motion <PROFILE> --kick-relation <MODE> --native-randomization --scale-root <NOTE> --scale-name <MODE> --number-tracks --no-number-tracks
```
...
<ParamField path="--clear-clips-first">
...
<ParamField path="clear_clips_first" type="Boolean">
