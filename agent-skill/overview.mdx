...
The zip contains the same three files a directory install writes — `SKILL.md`, `references/command-reference.md` and `references/onboarding.md` — just archived instead of written to disk, so the unzipped contents are identical either way. Upload the resulting file in Claude Desktop under **Settings → Capabilities → Skills → Upload skill**.

## What gets written

Each install writes a `vibrai/` skill folder containing the skill body, a freshly-rendered command reference, and an onboarding reference:
...
└── references/
    ...
    └── onboarding.md             # first-run setup and troubleshooting, in depth
```
...
`onboarding.md` is the hand-authored depth companion to `SKILL.md` for the first-run cliff — getting a working Live set, activating the remote script, diagnosing a stuck bridge, and reading the errors a first session hits most often. `SKILL.md` says what to do next; `onboarding.md` is what to read once something is already broken.

Unlike the command reference it is embedded in the binary rather than rendered from it. Both files travel with the version you installed and neither is ever fetched, but only the command reference is generated at install time.
...
`skill package` has an MCP counterpart, though: the `package_claude_desktop_skill` tool writes the same zip (`SKILL.md` + `references/command-reference.md` + `references/onboarding.md`) to a path you choose, since producing a standalone file for manual upload — unlike writing into an agent's own config directory — has a sensible MCP shape.
