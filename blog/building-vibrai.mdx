...
```mermaid
flowchart LR
  claude["AI agent<br/>(Claude Desktop / Cursor / Claude Code)"] -- "MCP stdio" --> mcp["Vibrai.Mcp"]
  user["Musician (terminal)"] --> cli["vibrai CLI"]
  mcp --> engine["Vibrai.Engine<br/>song form · generators · arrangement"]
  cli --> engine
  engine --> bridge["CompositeAbletonBridge"]
  bridge -- "HTTP :3333" --> m4l["M4L device<br/>Node-for-Max · session view"]
  bridge -- "HTTP :3334" --> py["Python remote script<br/>arrangement envelopes"]
  m4l -- "LOM (main thread)" --> live["Ableton Live 12"]
  py -- "LOM (main thread)" --> live
```
