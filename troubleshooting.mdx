...
<AccordionGroup>
  ...
  <Accordion title="uninstall refuses to run because your license expired">
    Both `vibrai uninstall` (CLI) and the `uninstall` MCP tool are license-gated with no exemption — an expired or unverifiable license blocks them like any other command, so there is no automated path out. Remove Vibrai by hand instead, in this order.
